...
public class RockPaperScissorsFlowTest {
    ...
    public void happyTest() throws Exception  {
        ...
        CordaFuture<UniqueIdentifier> futureAccept = b.startFlow(acceptFlow);
        network.runNetwork();
        ...
        for (StateAndRef<LinearState> linearState : linearStates) {
            System.out.println("STATE node a "+linearState.getState().getData().getClass()+" "+linearState.getState().getData().getLinearId());
        }
        states = b.getServices().getVaultService().queryBy(LinearState.class);
        linearStates = states.getStates();
        for (StateAndRef<LinearState> linearState : linearStates) {
            System.out.println("STATE node b "+linearState.getState().getData().getClass()+" "+linearState.getState().getData().getLinearId());
        }
        states = c.getServices().getVaultService().queryBy(LinearState.class);
        linearStates = states.getStates();
        for (StateAndRef<LinearState> linearState : linearStates) {
            System.out.println("STATE node c "+linearState.getState().getData().getClass()+" "+linearState.getState().getData().getLinearId());
        }
    }
}
